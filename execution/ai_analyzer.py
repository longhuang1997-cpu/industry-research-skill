...
class AIAnalyzer:
    ...
    def _build_analysis_prompt(self,
        ...
        prompt = f"""你是一位资深的行业研究分析师，拥有10年以上的{industry}行业研究经验。

请针对【{industry}】行业，使用{framework_name}进行深度分析。

# 关键要求
1. 必须针对【{industry}】行业的具体特征，不要使用泛泛的通用分析
2. 提供具体的数据、案例或事实支撑
3. 指出该行业独有的机会和风险
4. 给出可执行的战略建议
...
# 输出格式
直接输出一段150-200字的专业分析结论，包含：
- 该行业的核心特征（2-3个关键点）
- 主要机会或风险（具体且可量化）
- 3条可执行的战略建议

不要输出任何前缀、标题或解释，直接开始分析内容。

示例参考格式（请勿照抄，需根据{industry}行业实际情况分析）：
"{industry}行业[具体特征1]，[具体特征2]。核心机会在于[具体机会+数据]，但面临[具体风险]的挑战。建议：1）[具体建议1]；2）[具体建议2]；3）[具体建议3]。"
"""
        return prompt
